...
i32 main() {
    read(n,q);
    _rep(i,1,n) readstr(arr[i].s),read(arr[i].v),arr[i].i=i;
    _sorta(arr,1,n);
    _rep(i,1,n) {
        if (arr[i].v==arr[i-1].v) pre[i]=pre[i-1];
        else pre[i]=i;
    }
    _rrep(i,n,1) {
        if (arr[i].v==arr[i+1].v) suf[i]=suf[i+1];
        else suf[i]=i;
    }
    while (q--) {
        int x;
        ...
        while (l<r) {
            int mid=(l+r>>1)+1;
            if (arr[mid].v<=x) l=mid;
            else r=mid-1;
        }
        if (arr[r].v==x) {
            l=pre[l];
            r=suf[r];
            _rep(i,l,r) writestrsp(arr[i].s); putchar(10);
        } else {
            if (r==0) writestrln(arr[r+1].s);
            else {
                int nex=r+1;
                r=pre[r];
                if (nex==n+1) writestrln(arr[r].s);
                else if (abs(arr[r].v-x)==abs(arr[nex].v-x)) {
                    if (arr[r].i<arr[nex].i) writestrln(arr[r].s);
                    else writestrln(arr[nex].s);
                } else if (abs(arr[r].v-x)<abs(arr[nex].v-x)) writestrln(arr[r].s);
                else writestrln(arr[nex].s);
            }
        }
    }
