...
public:
    ...
    inline void build(int u,int l,int r) {
        tr[u].l=l,tr[u].r=r,tr[u].p=l,tr[u].v=0;
        if (l==r) return;
        int mid=l+r>>1;
...
SEG<val,pushup,pushdown,int,modify,0> seg(N);

inline void process(int i,int g) {
    int R=l[i];
    int L=r[i]-d;
    chmax(L,0),chmin(R,t-1);
...
i32 main() {
    ...
    while (q--) {
        ...
        if (op==1) {
            ...
            // cout<<"query: "<<0<<" "<<t-d<<endl;
            auto ret=seg.query(1,0,t-d);
            writespln(ret.p,ret.v);
        }
    }
    return 0;
}
