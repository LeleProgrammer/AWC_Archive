...
i32 main() {
    read(n,q);
    readstr(p);
    _rep(i,1,n) readstr(s[i]);
    pw[0]=1;
    _rep(i,1,(int)p.size()) pw[i]=pw[i-1]*P;
    _rep(i,1,n) {
        int l=0;
        u64 cur=0;
        _rep(r,0,(int)s[i].size()-1) {
            cur=cur*P+s[i][r]-'A';
            if (r-l+1>p.size()) cur-=pw[p.size()]*(s[i][l++]-'A');
            if (r-l+1==p.size()) mp[i][cur]++;
        }
    }
    u64 cur=0;
    _rep(i,0,(int)p.size()-1) {
        cur=cur*P+p[i]-'A';
        pre[i+1]=cur;
    }
    cur=0;
    _rrep(i,(int)p.size()-1,0) {
        cur=cur*P+p[i]-'A';
        suf[i+1]=cur;
    }
    int m=p.size();
    // _rep(i,1,m) cout<<"pre: "<<i<<" "<<pre[i]<<endl;
    // _rep(i,1,m) cout<<"suf: "<<i<<" "<<suf[i]<<endl;
    while (q--) {
        int i,l,r;
        read(i,l,r);
        u64 cur=pre[m];
        cur-=pre[r]*pw[m-r]-pre[l-1]*pw[m-(l-1)];
        cur+=suf[l]*pw[m-r]-suf[r+1]*pw[m-(l-1)];
        // cout<<"dec: "<<pre[r]*pw[m-r]-pre[l-1]*pw[m-(l-1)]<<endl;
        // cout<<"inc: "<<suf[l]*pw[m-r]-suf[r+1]*pw[m-(l-1)]<<endl;
        // cout<<"test: "<<m-r<<" "<<m-(l-1)<<" "<<cur<<endl;
        writeln(mp[i][cur]);
    }
    return 0;
}