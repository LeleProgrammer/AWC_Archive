...
inline int qmax(int i,int j) {
    int g=log2(k);
    return max4(st[i][j][g],st[i+k-(1<<g)][j][g],st[i][j+k-(1<<g)][g],st[i+k-(1<<g)][j+k-(1<<g)][g]);
}

inline int qsum(int i,int j) {
    return pre[i+k-1][j+k-1]-pre[i+k-1][j-1]-pre[i-1][j+k-1]+pre[i-1][j-1];
}

i32 main() {
    read(n,k);
    _rep(i,1,n) _rep(j,1,n) read(a[i][j]);
    _rep(i,1,n) _rep(j,1,n) st[i][j][0]=a[i][j];
    _rep(t,1,K-1) _rep(i,1,n) _rep(j,1,n) {
        st[i][j][t]=st[i][j][t-1];
        int len=(1<<(t-1));
        if (i+len<=n) chmax(st[i][j][t],st[i+len][j][t-1]);
        if (j+len<=n) chmax(st[i][j][t],st[i][j+len][t-1]);
        if (i+len<=n && j+len<=n) chmax(st[i][j][t],st[i+len][j+len][t-1]);
    }
    _rep(i,1,n) _rep(j,1,n) pre[i][j]=pre[i-1][j]+pre[i][j-1]-pre[i-1][j-1]+a[i][j];
    _rep(i,1,n-k+1) _rep(j,1,n-k+1) chmax(ans,qsum(i,j)-qmax(i,j));
    write(ans);
    return 0;
}