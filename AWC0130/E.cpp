...
inline void modify(int u,int l,int r,int k) { // chmin
    if (tr[u].mx<=k) return;
    if (tr[u].l==tr[u].r) {
        st.erase(pii{tr[u].mx,tr[u].l});
        tr[u].mx=k;
        st.emplace(pii{tr[u].mx,tr[u].l});
    } else {
        int mid=tr[u].l+tr[u].r>>1;
        if (l<=mid) modify(u<<1,l,r,k);
        if (r>mid) modify(u<<1|1,l,r,k);
...
i32 main() {
    ...
    _rep(i,1,n) read(h[i]);
    build(1,1,n);
    _rep(i,1,n) st.emplace(pii{h[i],i});
    while (!st.empty()) {
        auto t=st.begin(); st.erase(t);
        int num=t->first,pos=t->second;
        int l=max(1LL,pos-k+1);
        int r=min(n,pos+k-1);
        modify(1,l,r,num+d);
    }
    int ans=0;
    _rep(i,1,n) ans+=query(1,i);
    write(ans);
    return 0;
}