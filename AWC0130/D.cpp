...
inline void dfs(int x,int y,bool& res) {
    vis[x][y]=true;
    _rep(g,0,3) {
        int nx=x+dx4[g];
        int ny=y+dy4[g];
...
int main() {
    ...
    while (n--) {
        int a,b,c,d;
        read(a,b,c,d);
        _rep(i,a,b) p[i][c]^=1,p[i][d+1]^=1;
    }
    _rep(i,1,h) {
        _rep(j,1,w) p[i][j]^=p[i][j-1];
    }
    read(m);
    while (m--) {
        int a,b,c,d;
        read(a,b,c,d);
        int ans1=0;
        _rep(i,a,b) _rep(j,c,d) ans1+=p[i][j];
        writesp(ans1);
        _rep(i,a,b) _rep(j,c,d) vis[i][j]=false;
        int ans2=0;
        A=a,B=b,C=c,D=d;
        _rep(i,a,b) _rep(j,c,d) if (p[i][j] && !vis[i][j]) {
            bool res=true;
            dfs(i,j,res);
            ans2+=res;
        }
