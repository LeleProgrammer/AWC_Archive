...
i32 main() {
    ...
    _rep(i,0,n-2) read(w[i]);
    _rep(i,0,N-1) _rep(j,0,70000) _rep(t,0,N-1) f[i][j][t]=-inf;
    _rep(i,0,n-1) f[0][flpbit(0,i)][i]=0;
    _rep(i,1,n-1) {
        _for(j,vec[i+1]) {
            _rep(a,0,n-1) if (kthbit(j,a)) {
                int k=clrbit(j,a);
                _rep(b,0,n-1) if (kthbit(k,b)) {
                    chmax(f[i][j][a],f[i-1][k][b]+abs(p[a]-p[b])*w[i-1]);
                }
            }
        }
    }
    int ans=-inf;
    _rep(i,0,n-1) chmax(ans,f[n-1][flpbit(0,n)-1][i]);
    write(ans);
    return 0;
}