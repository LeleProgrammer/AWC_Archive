...
i32 main() {
    ...
    _rep(i,1,n) read(a[i],d[i]);
    _rep(i,1,q) read(qs[i].t),qs[i].i=i;
    sort(qs+1,qs+1+q);
    _rep(i,1,n) {
        if (!d[i]) {
            if (a[i]<=k) mp[0].emplace_back(i);
            continue;
        }
        int cnt=(a[i]-k)/d[i];
        if ((a[i]-k)%d[i]) cnt++;
    ...
    _rep(i,1,q) {
        ...
        while (vec.back()<=dest) {
            ...
            _for(v,mp[cur]) {
                if (v-1>=1 && flag[v-1]) cnt-=calc(sz[find(v-1)]),merge(v,v-1);
                if (v+1<=n && flag[v+1]) cnt-=calc(sz[find(v+1)]),merge(v,v+1);
                cnt+=calc(sz[find(v)]);
                flag[v]=true;
            }
        }
