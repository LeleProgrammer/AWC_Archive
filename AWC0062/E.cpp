...
SEG<val,pushup,nullptr,int,modify,0> seg(N);

i32 main() {
    read(n,q);
    _rep(i,1,n) read(x[i],b[i]);
    _rep(i,1,n) {
        mp[x[i]-b[i]+1]++;
        mp[x[i]+1]-=2;
        mp[x[i]+b[i]+1]++;
    ...
    int cur=0;
    _rep(i,-200000,200000) mp[i]=mp[i-1]+mp[i];
    _rep(i,-200000,200000) mp[i]=mp[i-1]+mp[i];
    _rep(i,0,200000) seg.modify(1,i,i,mp[i]);
    while (q--) {
        int l,r;
        read(l,r);
        writeln(seg.query(1,l,r).v);
    }
    return 0;
}
