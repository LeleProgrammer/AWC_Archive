...
i32 main() {
    ...
    readstr(s+1);
    readstr(t+1);
    _rep(i,1,l) pres[i]=pres[i-1]*P+s[i]-'0'+1;
    _rep(i,1,m) pret[i]=pret[i-1]*P+t[i]-'0'+1;
    _rrep(i,l,1) sufs[i]=sufs[i+1]+pw[l-i]*(s[i]-'0'+1);
    _rrep(i,m,1) suft[i]=suft[i+1]+pw[m-i]*(t[i]-'0'+1);
    // cout<<"pres: "; _rep(i,1,l) cout<<pres[i]<<" "; cout<<endl;
    // cout<<"sufs: "; _rep(i,1,l) cout<<sufs[i]<<" "; cout<<endl;
    // cout<<"pret: "; _rep(i,1,m) cout<<pret[i]<<" "; cout<<endl;
    // cout<<"suft: "; _rep(i,1,m) cout<<suft[i]<<" "; cout<<endl;
    int ans=0;
    _rep(i,1,min(l,m)) if (sufs[l-i+1]==pret[i]) ans=i;
    _rep(i,1,min(l,m)) if (suft[m-i+1]==pres[i]) chmax(ans,i);
    write(ans);
    return 0;
}