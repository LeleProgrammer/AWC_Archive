...
inline void csuf(const string& s) {
    int cur=0;
    u64 hs=0;
    _rrep(i,(int)s.size()-1,0) {
        hs+=pw[cur]*(s[i]-'a'+1);
        suf[i]=hs;
        cur++;
    }
...
inline void cpre(const string& s) {
    u64 hs=0;
...
i32 main() {
    ...
    int ans=0;
    _rep(i,1,n) ans+=s[i].size();
    _rep(i,2,n) {
        csuf(s[i-1]),cpre(s[i]);
        int l=0;
        _rep(j,1,min((int)s[i-1].size()-1,(int)s[i].size()-1)) if (suf[(int)s[i-1].size()-j]==pre[j-1]) l=j;
        ans-=l;
    }
    write(ans);
    return 0;
