...
inline void dfs2(int u) {
    ...
    do {
        dfs2(u+1);
    } while (next_permutation(_all(vec[u])));
}
...
i32 main() {
    ...
    _rep(i,1,n) vec[i].clear();
    dfs(1,0);
    dfs2(1);
    _for(s,st) if (st2.count(s)) ans.emplace_back(s);
    _sort(ans);
    writeln(ans.size());
    _for(s,ans) writestrln(s);
    return 0;
}