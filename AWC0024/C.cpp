...
int main() {
    read(n,m);
    _rep(i,1,n) _rep(j,1,m) scanf(" %c",&s[i][j]);
    _rep(i,1,n) {
        memset(cnt,0,sizeof(cnt));
        _rep(j,1,m) cnt[s[i][j]-'a']++;
        _rep(j,1,m) if (cnt[s[i][j]-'a']==1) ok[i][j]++;
    }
    _rep(j,1,m) {
        memset(cnt,0,sizeof(cnt));
        _rep(i,1,n) cnt[s[i][j]-'a']++;
        _rep(i,1,n) if (cnt[s[i][j]-'a']==1) ok[i][j]++;
    }
    string ans;
    _rep(i,1,n) _rep(j,1,m) if (ok[i][j]==2) ans.push_back(s[i][j]);
    writestr(ans);
    return 0;
}