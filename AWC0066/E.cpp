...
int c[20];
int deg[20][N];
int cnt[20][N],sum[20][N];
bool edg[20][20];

inline void solve() {
    queue<pii> q;
    cnt[s][setbit(0,s)]=1,sum[s][setbit(0,s)]=c[s];
    _rep(i,0,n-1) _rep(j,0,(1<<n)-1) if (kthbit(j,i) && !deg[i][j]) q.emplace(pii{i,j});
    while (!q.empty()) {
        auto t=q.front(); q.pop();
        int u=t.first,sta=t.second;
        // cout<<"test: "<<u<<" "<<sta<<": "<<cnt[u][sta]<<" "<<sum[u][sta]<<endl;
        _rep(v,0,n-1) if (edg[u][v] && !kthbit(sta,v)) {
            int nex=setbit(sta,v);
            cnt[v][nex]+=cnt[u][sta];
            sum[v][nex]+=sum[u][sta]+cnt[u][sta]*c[v];
            if (!--deg[v][nex]) q.emplace(pii{v,nex});
        }
    }
}
...
i32 main() {
    read(n,m,s,t);
    s--,t--;
    _rep(i,0,n-1) read(c[i]);
    while (m--) {
        int u,v;
        read(u,v);
        u--,v--;
        edg[u][v]=edg[v][u]=true;
        _rep(i,0,(1<<n)-1) if (kthbit(i,u) && !kthbit(i,v)) {
            deg[v][setbit(i,v)]++;
        }
        swap(u,v);
        _rep(i,0,(1<<n)-1) if (kthbit(i,u) && !kthbit(i,v)) {
    ...
    solve();
    double cnts=0,sums=0;
    _rep(i,0,(1<<n)-1) if (kthbit(i,s) && kthbit(i,t)) {
        cnts+=cnt[t][i];
        sums+=sum[t][i];
    }
    printf("%.12lf",sums/cnts);
    return 0;
}