...
i32 main() {
    read(n);
    _rep(i,1,n) read(h[i],w[i],s[i]);
    int ans=gcd(h[1],w[1]);
    _rep(i,1,n) ans=gcd(ans,gcd(h[i],w[i]));
    int res=h[1]*w[1]/s[1];
    _rep(i,1,n) res=gcd(res,h[i]*w[i]/s[i]);
    int ret=0;
    _rep(i,1,sqrt(ans)) {
        if (ans%i==0) {
            if (res%(i*i)==0) chmax(ret,i);
            if (res%(ans/i*ans/i)==0) chmax(ret,ans/i);
        }
    }
    write(ret);
    return 0;
