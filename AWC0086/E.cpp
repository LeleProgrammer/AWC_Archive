...
inline int solve() {
    ...
    _rep(i,1,n) _rep(j,1,m) {
        ...
        if (s[i]==t[j]) chmax(f[i][j],f[i-1][j-1]+1);
    }
    return f[n][m];
}
...
i32 main() {
    ...
    readstr(t+1),m=strlen(t+1);
    read(k);
    pii g[]={{0,1},{1,1},{0,n},{1,m}};
    int ans=0;
    _rep(tt,0,(1<<4)-1) if (popc(tt)<=k) {
        bool ok=true;
        _rep(i,0,3) if (kthbit(tt,i)) {
            int op=g[i].first;
            int pos=g[i].second;
            if (!op && ((pos==1 && s[pos]==s[pos+1]) || (pos==n && s[pos]==s[pos-1]) || n==1));
            else if (op && ((pos==1 && t[pos]==t[pos+1]) || (pos==m && t[pos]==t[pos-1]) || m==1));
            else ok=false;
        }
        if (!ok) continue;
        _rep(i,0,3) if (kthbit(tt,i)) {
