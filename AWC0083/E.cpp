...
i32 main() {
    read(n,k);
    _rep(i,0,n-1) read(p[i]);
    _rep(i,0,n-1) _rep(j,0,n-1) read(c[i][j]);
    _rep(i,0,N-1) _rep(t,0,N-1) _rep(j,0,M-1) f[i][t][j]=-inf;
    int ans=-inf;
    _rep(st,0,n-1) {
        f[st][st][setbit(0,st)]=p[st];
        _rep(t,0,(1<<n)-1) if (popc(t)!=1) _rep(i,0,n-1) if (kthbit(t,i)) _rep(las,0,n-1) if (las!=i && kthbit(t,las)) {
            chmax(f[st][i][t],f[st][las][clrbit(t,i)]+p[i]+c[i][las]);
            if (popc(t)==k) chmax(ans,f[st][i][t]+c[st][i]);
        }
    }
    write(ans);
    return 0;
