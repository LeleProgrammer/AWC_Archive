...
i32 main() {
    ...
    _rep(t,1,n) {
        _rep(i,1,m) if (!arr[t][i]) {
            int j=i;
            while (j+1<=m && !arr[t][j+1]) j++;
            _rep(k,i,j) add(get(t,k),get(t,j),1);
            i=j;
        }
        _rrep(i,m,1) if (!arr[t][i]) {
            int j=i;
            while (j-1>=1 && !arr[t][j-1]) j--;
            _rrep(k,i,j) add(get(t,k),get(t,j),1);
            i=j;
        }
    }
    _rep(t,1,m) {
        _rep(i,1,n) if (!arr[i][t]) {
            int j=i;
            while (j+1<=n && !arr[j+1][t]) j++;
            _rep(k,i,j) add(get(k,t),get(j,t),1);
            i=j;
        }
        _rrep(i,n,1) if (!arr[i][t]) {
            int j=i;
            while (j-1>=1 && !arr[j-1][t]) j--;
            _rrep(k,i,j) add(get(k,t),get(j,t),1);
            i=j;
        }
    }
    dijkstra(get(1,1));
