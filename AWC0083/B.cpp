...
i32 main() {
    ...
    while (q--) {
        ...
        if (op==1) {
            ...
            _rep(i,a,b) {
                __float128 l=0;
                _rep(j,1,i-1) chmax(l,(__float128)h[j]/(i-j));
                __float128 r=0;
                _rep(j,i+1,n) chmax(r,(__float128)h[j]/(j-i));
                __float128 v=1.0/(1.0+l+r);
                if (v-ans>eps) ans=v,res=i;
            }
            writeln(res);
        }
    }
