...
SEG<val,pushup,nullptr,int,modify> seg(N);

i32 main() {
    read(n,k);
    _rep(i,1,n) read(a[i]),seg.modify(1,i,i,a[i]);
    a[n+1]=1e18,seg.modify(1,n+1,n+1,a[n+1]);
    int ans=0;
    _rep(i,1,n) {
        int l=i,r=n+1;
        while (l<r) {
            int mid=(l+r>>1)+1;
            auto ret=seg.query(1,i,mid);
            if (ret.mx-ret.mn<k) l=mid;
            else r=mid-1;
        }
        int le=l;
        auto g=seg.query(1,i,le);
        if (g.mx-g.mn<k) le++;
        l=i,r=n+1;
        while (l<r) {
            int mid=(l+r>>1)+1;
        ...
        int ri=l;
        auto ret=seg.query(1,i,le);
        if (ret.mx-ret.mn!=k) continue;
        ans+=ri-le+1;
    }
    write(ans);
    return 0;
