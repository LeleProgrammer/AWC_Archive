...
i32 main() {
    read(n);
    _rep(i,1,n) read(arr[i].s,arr[i].c),nums.emplace_back(arr[i].s),nums.emplace_back(arr[i].c);
    _sort(nums),_unique(nums);
    _rep(i,1,n) arr[i].s=LB(nums,arr[i].s)+1,arr[i].c=LB(nums,arr[i].c)+1;
    _sorta(arr,1,n);
    int ans=0;
    _rrep(i,n,1) {
        int j=i;
        while (j-1>=1 && arr[j-1].s==arr[i].s) j--;
        _rep(k,j,i) tr.modify(arr[k].s,1);
        _rep(k,j,i) ans+=tr.query(arr[k].c-1);
        _rep(k,j,i) if (arr[k].c>arr[k].s) ans--;
        i=j;
    }
    write(ans);
    return 0;
