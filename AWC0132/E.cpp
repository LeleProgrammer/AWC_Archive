...
i32 main() {
    read(n,k);
    _rep(i,1,n) read(w[i]),cnt[w[i]]++;
    _rep(i,1,N-1) p[i]=i;
    _rep(i,k,N-1) _reps(j,i,N-1,j+=i) if (cnt[j]) merge(i,j);
    _rep(i,1,N-1) ans[find(i)]+=cnt[i]*i;
    int res=0;
    _rep(i,k,N-1) chmax(res,ans[i]);
    _rep(i,1,k-1) if (cnt[i]) chmax(res,i);
    // _rep(i,1,25) cout<<"test: "<<i<<" "<<find(i)<<endl;
    write(res);
    return 0;
}