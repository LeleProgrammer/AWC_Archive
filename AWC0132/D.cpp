...
i32 main() {
    ...
    while (q--) {
        ...
        int anc=lca(u,v);
        vector<int> vec;
        while (u!=anc) vec.emplace_back(u),u=LCA::f[u][0];
        vec.emplace_back(anc);
        vector<int> v2;
        while (v!=anc) v2.emplace_back(v),v=LCA::f[v][0];
        reverse(_all(v2));
        _for(t,v2) vec.emplace_back(t);
        reverse(_all(vec));
        vector<int> dp;
        _for(t,vec) {
            int v=arr[t];
            int pos=LB(dp,v);
            if (pos==dp.size()) dp.emplace_back(-1);
            dp[pos]=v;
        }
        writeln(dp.size());
    }
