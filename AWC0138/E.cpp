...
i32 main() {
    read(n,m,k,t);
    _rep(i,1,n) readstr(s[i]+1);
    while (k--) {
        int ttt=t;
        _rep(i,1,n) _rep(j,1,m) nex[i][j]={-1,-1},vis[i][j]=0;
        int x,y,l;
        read(x,y,l);
        readstr(op);
        vector<pii> his;
        his.emplace_back(x,y),vis[x][y]=true;
        while (t>=l) {
            auto pos=get(x,y);
            int nx=pos.first,ny=pos.second;
            if (vis[nx][ny]) {
                int k;
                _rep(i,0,(int)his.size()-1) if (his[i].first==nx && his[i].second==ny) {
                    k=i;
                    break;
                }
                int len=(int)his.size()-k;
                int cnt=t/l;
                // cout<<"test: "<<k<<" "<<his.size()-1<<" "<<cnt<<endl;
                t%=l;
                cnt%=len;
                if (!cnt) cnt+=len;
                int dest=k+cnt-1;
                x=his[dest].first,y=his[dest].second;
                // cout<<"Dest: "<<dest<<": "<<x<<" "<<y<<endl;
                break;
            }
            t-=l;
            x=pos.first,y=pos.second;
            his.emplace_back(pii{x,y});
            vis[x][y]=true;
            // cout<<"now: "<<x<<" "<<y<<endl;
        }
        _for(c,op) {
