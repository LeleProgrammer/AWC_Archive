...
i32 main() {
    read(n,w,b);
    _rep(i,1,n) read(arr[i].d,arr[i].c);
    _rep(i,0,1) _rep(j,0,P-1) _rep(k,0,2) f[i][j][k]=-inf;
    int cur=1,las=0;
    f[cur][0][0]=0;
    _rep(i,1,n) {
        swap(cur,las);
        _rep(j,0,w) f[cur][j][0]=f[cur][j][1]=f[cur][j][2]=-inf;
        _rrep(j,w,arr[i].c) {
            chmax(f[cur][j][1],f[las][j-arr[i].c][0]+arr[i].d);
            chmax(f[cur][j][2],max(f[las][j-arr[i].c][2]+arr[i].d+b,f[las][j-arr[i].c][1]+arr[i].d+b+b));
        }
        _rep(j,0,w) chmax(f[cur][j][0],max3(f[las][j][0],f[las][j][1],f[las][j][2]));
    }
    int ans=0;
    _rep(i,0,w) chmax(ans,max3(f[cur][i][0],f[cur][i][1],f[cur][i][2]));
    write(ans);
    return 0;
}