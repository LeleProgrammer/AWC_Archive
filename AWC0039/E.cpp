...
namespace DSeg {
    ...
    inline int query(int u,int l,int r,int L,int R) {
        ...
        int mid=L+R>>1;
        int ans=0;
        if (l<=mid) ans+=query(tr[u].l,l,r,L,mid);
        if (r>mid) ans+=query(tr[u].r,l,r,mid+1,R);
        return ans;
    }
}
...
inline void build(int u,int l,int r) {
    tr[u]={l,r,0};
    _rep(i,l,r) _for(g,vec[i]) DSeg::insert(tr[u].root,1,1e9,g);
    if (l==r) return;
    int mid=l+r>>1;
    build(u<<1,l,mid);
    build(u<<1|1,mid+1,r);
...
inline int query(int u,int l,int r,int t) {
    if (tr[u].l>=l && tr[u].r<=r) return DSeg::query(tr[u].root,t,1e9,1,1e9);
    int ans=0;
    int mid=tr[u].l+tr[u].r>>1;
    if (l<=mid) ans+=query(u<<1,l,r,t);
    if (r>mid) ans+=query(u<<1|1,l,r,t);
    return ans;
}

i32 main() {
    read(n,m,q,k);
    _rep(i,1,m) read(arr[i].s,arr[i].d),vec[arr[i].s].emplace_back(arr[i].d);
    build(1,1,n);
    while (q--) {
        int l,r,t;
        read(l,r,t);
