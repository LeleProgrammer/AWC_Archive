...
inline void solve(int l,int r,vector<node>& vec) {
    vector<node> lvec,mvec,rvec;
    int mid=l+r>>1;
    _for(u,vec) {
        if (u.r<=mid-1) lvec.emplace_back(u);
        else if (u.l>=mid+1) rvec.emplace_back(u);
        else mvec.emplace_back(u);
    }
    vec.clear();
    _rep(j,0,M-1) f[mid+1][j]=0;
    _rrep(i,mid,l) _rep(j,0,M-1) {
        if (i==mid) f[i][j]=0;
        else f[i][j]=f[i+1][j];
        if (j-h[i]>=0) chmax(f[i][j],f[i+1][j-h[i]]+w[i]);
        if (j) chmax(f[i][j],f[i][j-1]);
    }
    _rep(j,0,M-1) g[mid][j]=0;
    _rep(i,mid+1,r) _rep(j,0,M-1) {
        if (i==mid+1) g[i][j]=0;
        else g[i][j]=g[i-1][j];
        if (j-h[i]>=0) chmax(g[i][j],g[i-1][j-h[i]]+w[i]);
        if (j) chmax(g[i][j],g[i][j-1]);
    }
    _for(u,mvec) _rep(j,0,u.t) chmax(ans[u.i],f[u.l][j]+g[u.r][u.t-j]);
    if (!lvec.empty()) solve(l,mid,lvec);
    if (!rvec.empty()) solve(mid+1,r,rvec);
}

i32 main() {
    int tmp;
    read(n,tmp,m);
    _rep(i,1,n) read(h[i],w[i]);
    _rep(i,1,m) read(qs[i].l,qs[i].r,qs[i].t),qs[i].i=i;
    vector<node> st;
    _rep(i,1,m) st.emplace_back(qs[i]);
    solve(1,n,st);
