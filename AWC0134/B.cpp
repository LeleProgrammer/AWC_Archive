...
i32 main() {
    ...
    _rep(i,1,n) {
        ...
        _rep(j,1,7) {
            if (l<=r) {
                if (j>=l && j<=r) tas[j].emplace_back(i),ans[i]+=w;
            } else {
                if (j<=r || j>=l) tas[j].emplace_back(i),ans[i]+=w;
            }
        }
    }
    while (m--) {
    ...
    //     putchar(10);
    // }
    _rep(i,1,7) _for(t,tas[i]) ans[t]+=cnt[i];
    _rep(i,1,n) writeln(ans[i]);
    return 0;
}