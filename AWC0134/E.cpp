...
i32 main() {
    ...
    _rep(r,1,n) {
        cur+=a[r];
        while (cur>p) cur-=a[l],l++;
        if (cur==p && l<=r) vec.emplace_back(l,r);
    }
    _rep(i,1,q) read(qs[i].l,qs[i].r),qs[i].i=i;
    _sorta(qs,1,q);
    sort(_all(vec),[&](const pii& a,const pii& b) {
    ...
    cur=1;
    _rep(i,1,n) {
        while (!vec.empty() && vec.back().second==i) {
            int l=vec.back().first,r=vec.back().second;
            vec.pop_back();
            sum.modify(l,r-l+1);
            cnt.modify(l,1);
        }
        while (cur<=q && qs[cur].r==i) {
            int l=qs[cur].l,r=qs[cur].r,id=qs[cur].i; cur++;
            int C=cnt.query(N-1)-cnt.query(l-1);
            int T=sum.query(N-1)-sum.query(l-1);
            ans[id]={C,T};
        }
    }
    _rep(i,1,q) writespln(ans[i].first,ans[i].second);
    return 0;
}