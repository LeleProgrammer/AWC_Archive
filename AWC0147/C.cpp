...
i32 main() {
    read(n,R,k);
    _rep(i,1,n) read(d[i]);
    _rep(i,0,14) _rep(j,0,14) _rep(k,0,104) f[i][j][k]=inf;
    f[1][0][d[1]]=0;
    _rep(i,0,100) f[1][1][i]=0;
    _rep(i,2,n) {
        _rep(j,0,k) _rep(t,0,100) chmin(f[i][j][d[i]],f[i-1][j][t]+(abs(t-d[i])>R));
        _rep(j,1,k) _rep(p,0,100) _rep(t,0,100) chmin(f[i][j][p],f[i-1][j-1][t]+(abs(t-p)>R));
    }
    int ans=inf;
    _rep(i,0,k) _rep(j,0,100) chmin(ans,f[n][i][j]);
    write(ans);
    return 0;
}