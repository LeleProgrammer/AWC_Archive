...
SEG<val,pushup,nullptr,int,modify> seg[11];

i32 main() {
    read(n,m,k);
    _rep(i,0,k) seg[i].reserve(N);
    _rep(i,1,m) read(arr[i].l,arr[i].r);
    _rep(i,1,m) {
        nums.emplace_back(arr[i].l);
        nums.emplace_back(arr[i].r);
    }
    _sort(nums);
    _unique(nums);
    _sorta(arr,1,m);
    _rep(i,1,m) {
        ...
        _rep(j,1,k) {
            while (!mpa[j-1].empty() && mpa[j-1].begin()->first<l) {
                chmax(mx[j-1],mpa[j-1].begin()->second);
                mpa[j-1].erase(mpa[j-1].begin());
            }
            int res=mx[j-1]+r-l+1;
            int ret=seg[j-1].query(1,LB(nums,l)+1,N-1).mx+r;
            chmax(res,ret);
            chmax(mpa[j][r],res);
            seg[j].modify(1,LB(nums,r)+1,LB(nums,r)+1,res-r);
        }
    }
    int ans=0;
    _rep(i,0,k) chmax(ans,mx[i]);
    _rep(j,0,k) while (!mpa[j].empty()) {
        chmax(ans,mpa[j].begin()->second);
        mpa[j].erase(mpa[j].begin());
    }
    write(ans);
    return 0;
}