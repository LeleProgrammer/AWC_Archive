...
inline void merge(int x,int y) {
    ...
    cur-=mx[find(x)]+mx[find(y)];
    chmax(mx[find(x)],mx[find(y)]);
    cur+=mx[find(x)];
    p[find(y)]=find(x);
}
...
i32 main() {
    read(n,q);
    _rep(i,1,n) read(a[i],b[i]);
    _rep(i,1,q) read(qs[i].x),qs[i].i=i;
    _sorta(qs,1,q);
    _rep(i,1,n) vec.emplace_back(i);
    sort(_all(vec),[&](const int& i,const int& j) {
        return a[i]<a[j];
    });
    _rep(i,1,n) p[i]=i,mx[i]=b[i];
    _rep(i,1,q) {
        int x=qs[i].x,id=qs[i].i;
        while (!vec.empty() && a[vec.back()]>=x) {
