...
i32 main() {
    ...
    _rrep(i,n,1) {
        cnt.modify(nex[i],1);
        sum.modify(nex[i],nex[i]);
        while (cur<=q && arr[cur].l==i) {
            int l=arr[cur].l,r=arr[cur].r,id=arr[cur].i; ++cur;
            int c=cnt.query(n+1)-cnt.query(r);
            int s=sum.query(r);
            ans[id]=c*r+s-calc(l-1,r-1)-(n-r)*r;
            // cout<<"id: "<<id<<" "<<c<<" "<<s<<" "<<r<<endl;
        }
    }
