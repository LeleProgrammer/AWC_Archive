...
inline int solve(int n,int k) {
    _rep(i,0,9) f[0][i][0]=1;
    _rep(i,1,N-1) _rep(j,0,9) _rep(k,0,M-1) _rep(t,0,9) if (k-abs(j-t)>=0) f[i][j][k]+=f[i-1][t][k-abs(j-t)];
    vector<int> vec;
    while (n) {
    ...
    int ans=0;
    _rep(i,0,m-1) _rep(j,1,9) ans+=f[i][j][k];
    _rep(j,1,(int)vec.back()-1) ans+=f[m][j][k];
    int cur=0,las=vec.back();
    _rrep(i,m-1,0) {
        _rep(j,0,vec[i]-1) if (k-cur-abs(las-j)>=0) ans+=f[i][j][k-cur-abs(las-j)];
        cur+=abs(las-vec[i]);
        las=vec[i];
    }
    return ans;
