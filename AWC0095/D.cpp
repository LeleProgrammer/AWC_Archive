...
inline bool check(int i,int j) {
    if (arr[i].l>=arr[j].r) return true;
    if (arr[j].l>=arr[i].r) return true;
    return false;
}

i32 main() {
    read(n,k);
    _rep(i,0,n-1) read(arr[i].l,arr[i].r,arr[i].v);
    int ans=inf;
    _rep(t,0,(1<<n)-1) if (popc(t)==k) {
        int mx=0;
        _rep(p,0,(1<<n)-1) if (!(t&p)) {
            int sum=0;
            bool ok=true;
            _rep(i,0,n-1) _rep(j,0,n-1) {
                if (i==j) continue;
                if (!kthbit(p,i) || !kthbit(p,j)) continue;
                if (!check(i,j)) {
                    ok=false;
                    break;
                }
            ...
            if (!ok) continue;
            _rep(i,0,n-1) if (kthbit(p,i)) sum+=arr[i].v;
            chmax(mx,sum);
        }
        chmin(ans,mx);
    }
