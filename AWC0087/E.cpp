...
struct val {
    int l,r;
    int lmax,rmax,sum,sumf,max;
    val(): l(0),r(0),lmax(0),rmax(0),sum(0),sumf(inf),max(0) {}
};

inline void pushup(val& u,val& l,val& r) {
    u.sum=l.sum+r.sum;
    u.lmax=min(l.lmax,l.sumf+r.lmax);
    u.rmax=min(r.rmax,r.sumf+l.rmax);
    u.sumf=l.sumf+r.sumf;
    u.max=min4(l.max+r.sum,l.sum+r.max,l.sum+r.sum,l.sum+r.sum+l.rmax+r.lmax);
}
...
SEG<val,pushup,nullptr,pii,modify> seg(N);
...
i32 main() {
    ...
    _rep(i,1,n) seg.modify(1,i,i,pii{s[i],p[i]});
    while (q--) {
        int l,r;
        read(l,r);
        writeln(seg.query(1,l,r).max);
    }
    return 0;
}