...
inline void topo() {
    queue<int> q;
    _rep(i,1,n) if (!deg[i]) q.emplace(i);
    while (!q.empty()) {
        int u=q.front(); q.pop();
        vis[u]=true;
        _graph(i,u) if (!--deg[e[i]]) q.emplace(e[i]);
    }
}
...
i32 main() {
    ...
    read(n);
    _rep(i,1,n) p[i]=i;
    _rep(i,1,n) read(t[i]),add(t[i],i),deg[i]++,merge(i,t[i]);
    set<int> st;
    _rep(i,1,n) if (!vis[i]) st.emplace(find(i));
    write(n-(int)st.size());
    return 0;
}