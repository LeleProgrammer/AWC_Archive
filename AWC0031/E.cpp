...
i32 main() {
    ...
    read(n,k,m);
    _rep(i,1,n) read(b[i]);
    _rep(i,1,k) read(ww[i]);
    _rep(i,1,m) read(arr[i].a,arr[i].b,arr[i].c);
    int sumb=0;
    _rep(i,1,n) sumb+=b[i];
    read(q);
    bool ok=true;
    while (q--) {
        ...
        _rep(i,1,k) if (!down[i]) add_flow(S,n+i,ww[i]);
        _rep(i,1,m) {
            add_flow(arr[i].a,arr[i].b,arr[i].c);
            add_flow(arr[i].b,arr[i].a,arr[i].c);
        }
        _rep(i,1,n) add_flow(i,T,b[i]);
        int res=dinic();
        if (res<sumb) ok=false;
        if (!ok) puts("No");
        else puts("Yes");
    }
    return 0;
