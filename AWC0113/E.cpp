...
i32 main() {
    ...
    _rep(i,1,m) read(r[i]);
    _rep(i,1,m) if (!r[i]) _for(v,s[i]) fno[v]=true;
    _rep(i,1,m) if (r[i]) _for(v,s[i]) if (!fno[v]) val[v]=setbit(val[v],i-1);
    _rep(i,1,n) if (!fno[i]) f[val[i]]++;
    // _rep(i,1,n) if (!fno[i]) cout<<"val: "<<i<<" "<<val[i]<<endl;
    _rep(i,0,m-1) _rep(mask,0,(1<<m)-1) if (kthbit(mask,i)) f[mask]+=f[clrbit(mask,i)];
    int tot=0;
    _rep(i,1,m) if (r[i]) tot=setbit(tot,i-1);
    _rep(i,0,(1<<m)-1) f[i]=pw[f[i]];
    _rep(i,0,m-1) _rep(mask,0,(1<<m)-1) if (kthbit(mask,i)) (f[mask]-=f[clrbit(mask,i)])%=mod;
    int ans=f[tot];
    ans=(ans%mod+mod)%mod;
    // cout<<"test: "<<tot<<endl;
    write(ans);
