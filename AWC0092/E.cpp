...
SEG<val,pushup,nullptr,int,modify> seg(50005);

i32 main() {
    read(n,c,q);
    _rep(i,1,n) read(a[i]);
    _rep(i,1,n) {
        if (a[i]) seg.modify(1,i,i,a[i]);
        else tr.modify(i,1),seg.modify(1,i,i,inf);
    }
    while (q--) {
        int op;
        read(op);
        if (op==1) {
            int p,x;
            read(p,x);
            if (!a[p]) tr.modify(p,-1);
            a[p]=x;
            if (a[p]) seg.modify(1,p,p,a[p]);
            else tr.modify(p,1),seg.modify(1,p,p,inf);
        } else {
            int l,r,d;
            read(l,r,d);
            int ans=tr.query(r)-tr.query(l-1);
            int cur=l;
            while (cur<=r) {
                if (seg.query(1,cur,r).mn>d) break;
                int L=cur,R=r;
                while (L<R) {
                    int mid=L+R>>1;
                    if (seg.query(1,cur,mid).mn<=d) R=mid;
                    else L=mid+1;
                }
                ans++;
                d-=seg.query(1,R,R).mn;
                cur=R+1;
            }
            writeln(ans);
        }
