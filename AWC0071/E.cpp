...
inline void dcol(int u,int t) {
    col[u]=t;
    _graph(i,u) if (!~col[e[i]]) dcol(e[i],t^1);
}
...
i32 main() {
    ...
    read(n,m),S=n+1,T=n+2;
    _rep(i,1,m) read(arr[i].first,arr[i].second),add(arr[i].first,arr[i].second),add(arr[i].second,arr[i].first);
    _rep(i,1,n) if (!~col[i]) dcol(i,0);
    _rep(i,1,m) {
        if (col[arr[i].first]) swap(arr[i].first,arr[i].second);
        add_flow(arr[i].first,arr[i].second,1);
    }
    _rep(i,1,n) {
        if (!col[i]) add_flow(S,i,1);
        else add_flow(i,T,1);
    }
    writeln(dinic());
    return 0;
