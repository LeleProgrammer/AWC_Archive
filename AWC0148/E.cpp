...
i32 main() {
    read(n,m,k);
    _rep(i,1,n) read(t[i]),t[i]--;
    _rep(i,0,n) _rep(j,i+1,n+1) read(g[i][j]);
    _rep(i,0,N-1) _rep(j,0,N-1) _rep(k,0,P-1) dis[i][j][k]=inf;
    dis[0][0][0]=0;
    _rep(u,0,n) _rep(cnt,0,u) _rep(sta,0,(1<<m)-1) {
        if (dis[u][cnt][sta]==inf) continue;
        _rep(v,u+1,n+1) {
            int nex=sta;
            if (v!=n+1) nex=flpbit(nex,t[v]);
            int ct=cnt;
            if (v!=n+1) ct++;
            if (dis[v][ct][nex]>dis[u][cnt][sta]+g[u][v]) dis[v][ct][nex]=dis[u][cnt][sta]+g[u][v];
        }
    }
    int ans=inf;
    _rep(i,k,n) chmin(ans,dis[n+1][i][0]);
    if (ans==inf) ans=-1;
    write(ans);
    return 0;
}