...
constexpr inline int calc(int k) {
    vector<int> vec;
    while (k) vec.emplace_back(k%10),k/=10;
    int ans=0;
    _for(t,vec) ans+=t;
    _rep(i,0,(int)vec.size()-2) {
        if (vec[i]!=9) {
            vec[i]=9,vec[i+1]--;
            int j=i+1;
            while (j<=(int)vec.size()-1 && vec[j]<0) {
                vec[j]+=10;
                vec[j+1]--;
                ++j;
            }
...
inline int query(int i,int l,int r) {
    int t=log2(r-l+1);
    return max(mx[i][l][t],mx[i][r-(1<<t)+1][t]);
}

i32 main() {
    read(n,q);
    _rep(i,1,n) _rep(j,1,i) read(a[i][j]);
    _rep(i,1,n) {
        _rep(j,1,i) mx[i][j][0]=calc(a[i][j]);
        _rep(k,1,11) {
            _rep(j,1,i) {
                mx[i][j][k]=mx[i][j][k-1];
                if (j+(1<<(k-1))<=i) chmax(mx[i][j][k],mx[i][j+(1<<(k-1))][k-1]);
            }
        }
    }
    while (q--) {
        ...
        int ans=0;
        _rep(i,l,min(n,l+t)) chmax(ans,query(i,p,p+(i-l)));
        writeln(ans);
    }
    return 0;
}