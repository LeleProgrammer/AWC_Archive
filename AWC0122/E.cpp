...
struct que {
    int l,r,i;
    _sortf(que) {
        if (gt(a.l)!=gt(b.l)) return gt(a.l)<gt(b.l);
        else return a.r<b.r;
    }
} qs[N];
...
i32 main() {
    read(n,k,q),len=sqrt(k);
    _rep(i,1,n) read(arr[i].l,arr[i].r,arr[i].x);
    _rep(i,1,k) {
        int cur=i;
        _rep(j,1,n) if (arr[j].l<=cur && cur<=arr[j].r) cur=arr[j].x;
        g[i]=cur;
    }
    ...
    int i=1,j=0;
    _rep(t,1,q) {
        int l=qs[t].l,r=qs[t].r,id=qs[t].i;
        while (i<l) del(g[i++]);
        while (i>l) add(g[--i]);
        while (j<r) add(g[++j]);
        while (j>r) del(g[j--]);
