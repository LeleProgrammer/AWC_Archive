...
int n,k;
double f[N];
double suf[N],pre[N],p[N];

i32 main() {
    read(n,k);
    _rep(i,1,n) read(f[i]);
    _rep(i,1,n) p[i]=p[i-1]+f[i];
    pre[0]=suf[n+1]=-inf;
    _rep(i,1,n) pre[i]=max(pre[i-1],f[i]);
    _rrep(i,n,1) suf[i]=max(suf[i+1],f[i]);
    double ans=inf;
    _rep(i,k,n) {
        chmin(ans,max3(pre[i-k],suf[i+1],(p[i]-p[i-k])/k));
    }
    printf("%.12lf",ans);
    return 0;
}