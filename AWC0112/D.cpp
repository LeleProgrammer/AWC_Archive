...
i32 main() {
    read(n,k);
    _rep(i,1,n) read(arr[i].v,arr[i].w);
    _rep(j,0,N-1) _rep(i,0,M-1) f[j][i][0]=f[j][i][1]=-inf;
    f[0][0][0]=0;
    _rep(i,1,n) _rep(j,0,k) {
        f[i][j][0]=max(f[i-1][j][0],f[i-1][j][1]);
        if (j>=arr[i].w) f[i][j][1]=f[i-1][j-arr[i].w][0]+arr[i].v;
    }
    int ans=0;
    _rep(i,0,k) chmax(ans,max(f[n][i][0],f[n][i][1]));
    write(ans);
    return 0;
}