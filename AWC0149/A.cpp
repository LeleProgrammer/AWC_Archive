...
i32 main() {
    read(n,m,r,c);
    _rep(i,1,r) readstr(s[i]+1);
    _rep(i,1,n) _rep(j,1,m) t[i][j]='.';
    read(q);
    while (q--) {
        int a,b;
        read(a,b);
        _rep(i,1,r) _rep(j,1,c) if (a+i-1>=1 && b+j-1>=1 && a+i-1<=n && b+j-1<=m && s[i][j]=='#') t[a+i-1][b+j-1]='#';
    }
    _rep(i,1,n) {
        _rep(j,1,m) putchar(t[i][j]);
        putchar(10);
    }
    return 0;
}