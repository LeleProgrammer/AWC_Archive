...
i32 main() {
    ...
    _rep(i,0,k-1) read(g[i]);
    _rep(i,0,k-1) {
        SP::restore();
        dijkstra(g[i]);
        _rep(j,0,k-1) d[i][j]=dis[g[j]];
    }
    memset(dp,0x3f,sizeof(dp));
    SP::restore();
    dijkstra(1);
    _rep(i,0,k-1) dp[(1<<i)][i]=dis[g[i]];
    _rep(i,0,(1<<k)-1) if (popc(i)>=2) _rep(j,0,k-1) if (kthbit(i,j)) {
        _rep(t,0,k-1) if (t!=j && kthbit(i,t)) chmin(dp[i][j],dp[clrbit(i,j)][t]+d[t][j]);
    }
    SP::restore();
    dijkstra(n);
    int ans=inf;
    _rep(i,0,k-1) chmin(ans,dp[(1<<k)-1][i]+dis[g[i]]);
    write(ans);
    return 0;
}