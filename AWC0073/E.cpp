...
i32 main() {
    ...
    _rep(i,1,n) if (deg[i]&1) vec.emplace_back(i);
    if (vec.empty()) {
        write(sum);
        return 0;
    }
    _for(u,vec) {
        ...
        _for(v,vec) {
            d[u][v]=dis[v];
            // cout<<"d: "<<u<<" "<<v<<": "<<d[u][v]<<endl;
        }
    }
    memset(g,0x3f,sizeof(g));
    int p=vec.size();
    g[0]=0;
    _rep(t,0,(1<<p)-1) if (!(popc(t)&1)) {
        _rep(i,0,p-1) _rep(j,0,p-1) if (i!=j && !kthbit(t,i) && !kthbit(t,j)) {
            int nex=setbit(setbit(t,i),j);
            chmin(g[nex],g[t]+d[vec[i]][vec[j]]);
        }
    }
    write(g[(1<<p)-1]+sum);
    return 0;
