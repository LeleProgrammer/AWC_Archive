...
inline void init() {
    _rep(i,0,(1<<15)-1) vec[popc(i)].emplace_back(i);
    _rep(i,0,M-1) _rep(j,0,15) f[i][j]=inf;
}

i32 main() {
    init();
    read(n,m);
    _rep(i,0,N-1) _rep(j,0,N-1) g[i][j]=inf;
    _rep(i,1,n) g[i][i]=0;
    while (m--) {
        int a,b,c;
        read(a,b,c);
    ...
    read(s,k);
    _rep(i,0,k-1) read(t[i]);
    _rep(p,1,n) _rep(i,1,n) _rep(j,1,n) chmin(g[i][j],g[i][p]+g[p][j]);
    _rep(l,1,k) _for(i,vec[l]) {
        if (l==1) {
            _rep(j,0,k-1) if (kthbit(i,j)) f[i][j]=g[s][t[j]];
        } else {
            _rep(j,0,k-1) _rep(q,0,k-1) if (j!=q && kthbit(i,j) && kthbit(i,q)) chmin(f[i][j],f[clrbit(i,j)][q]+g[t[q]][t[j]]);
        }
    }
    int res=inf;
    _rep(i,0,k-1) chmin(res,f[(1<<k)-1][i]+g[t[i]][s]);
    if (res==inf) res=-1;
    write(res);
    return 0;
}