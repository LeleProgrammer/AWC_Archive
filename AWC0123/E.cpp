...
i32 main() {
    ...
    _sort(vec);
    _unique(vec);
    _rep(i,0,(int)vec.size()-1) id[vec[i]]=i;
    int len=vec.size();
    _rep(i,0,(int)vec.size()-1) {
        int u=vec[i];
        SP::restore();
        dijkstra(u);
        _rep(j,0,(int)vec.size()-1) d[id[u]][id[vec[j]]]=dis[vec[j]];
    }
    _rep(i,0,len-1) _rep(j,0,P-1) g[i][j]=inf;
    _rep(i,1,k) {
        _for(t,s[i]) val[i]=setbit(val[i],id[t]);
    }
    g[0][1]=0;
    _rep(sta,1,(1<<len)-1) _rep(i,0,len-1) {
        if (!kthbit(sta,i)) continue;
        _rep(j,0,len-1) chmin(g[j][setbit(sta,j)],g[i][sta]+d[i][j]);
    }
    int ans=0;
    _rep(i,0,len-1) _rep(sta,1,(1<<len)-1) if (g[i][sta]<=t) {
        int sum=0;
        _rep(j,1,k) if ((val[j]&sta)==val[j]) sum+=p[j];
        chmax(ans,sum); 
    }
    write(ans);
    return 0;
