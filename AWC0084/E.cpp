...
inline void chkmin(vector<int>& res,vector<int> ans) {
    _rep(i,0,l-1) {
...
int main() {
    ...
    _rep(i,1,l) read(b[i]);
    _rep(i,1,n) {
        _rep(j,1,l) read(tt[i][j]);
        _rep(j,1,k) read(aa[i][j]);
    }
    _rep(i,1,n) pe[i]=i;
    do {
        _rep(i,1,n) {
            _rep(j,1,l) t[i][j]=tt[pe[i]][j];
            _rep(j,1,k) a[i][j]=aa[pe[i]][j];
        }
        _rep(i,1,k) c[1][i]=i;
        _rep(i,2,n) _rep(j,1,k) c[i][j]=a[i-1][c[i-1][j]];
        _rep(i,1,l) _rep(j,1,k) {
            int sta=0;
            _rep(p,1,n) if (t[p][i]==c[p][j]) sta=setbit(sta,p-1);
            s[i][j]=sta;
        }
        _rrep(dest,(1<<n)-1,0) {
            if (popc(dest)<len) continue;
            bool ok=true;
            vector<int> ans;
            _rep(i,1,l) {
                if (b[i]) {
                    int nex=(dest&s[i][b[i]]);
                    if (nex!=dest) {
                        ok=false;
                        break;
                    }
            ...
            if (!ok) continue;
            if (popc(dest)==len) chkmin(res,ans);
            else res=ans;
            len=popc(dest);
        }
    } while (next_permutation(pe+1,pe+1+n));
    writeln(len);
    _for(g,res) writesp(g);
    return 0;
}