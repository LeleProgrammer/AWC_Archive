...
inline void dfs(int cur,int len,int con) {
    if (len==n) vec.emplace_back(cur);
    else {
        int las=(cur&1);
        if (con!=2) dfs(cur*2+las,len+1,con+1);
        dfs(cur*2+(las^1),len+1,1);
    }
}

inline bool check(int k) {
    _rep(i,0,n-3) if (kthbit(k,i)==kthbit(k,i+1) && kthbit(k,i)==kthbit(k,i+2)) return false;
    return true;
}
...
i32 main() {
    ...
    _rep(i,1,n) fr=fr*2+s[i]-'0';
    _rep(i,1,n) to=to*2+t[i]-'0';
    dfs(1,1,1),dfs(0,1,1);
    _for(k,vec) {
        _rep(i,0,n-1) if (check(flpbit(k,i))) q[k].emplace_back(flpbit(k,i));
        sort(_rall(q[k]));
        // cout<<k<<": ";
        // _for(t,q[k]) cout<<t<<" ";
        // cout<<endl;
    ...
    int ans=0;
    while (fr!=to) {
        while (!q[fr].empty() && vis[q[fr].back()]) q[fr].pop_back();
        if (q[fr].empty()) break;
        int nex=q[fr].back(); q[fr].pop_back();
        vis[nex]=true;
        fr=nex,ans++;
    }
    if (fr!=to) ans=-1;
