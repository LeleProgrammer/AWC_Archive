...
i32 main() {
    ...
    _rep(i,1,n) {
        int pos=LB(vec,a[i]);
        if (pos==(int)vec.size()) vec.emplace_back(-1);
        vec[pos]=a[i];
    }
    writeln(vec.size());
    _rrep(i,n,1) {
        if (mp.empty()) mp.emplace(pii{a[i],i});
        else {
            auto it=UB2(mp,a[i]);
            if (it==mp.begin()) mp.emplace(pii{a[i],i});
            else {
                it=prev(it);
                int num=it->first,pos=it->second;
                mp.erase(it);
                b[pos]=i;
                mp.emplace(pii{a[i],i});
            }
        }
    }
    _rep(i,1,n) writesp(b[i]);
    return 0;
}