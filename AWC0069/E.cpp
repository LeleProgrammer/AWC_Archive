...
i32 main() {
    read(n,q);
    _rep(i,1,n) read(a[i],d[i],v[i]);
    _rep(i,1,q) read(qs[i].l,qs[i].r,qs[i].t),qs[i].i=i;
    sort(qs+1,qs+1+q);
    _rep(i,1,n) vec[d[i]].emplace_back(i);
    int cur=1;
    _rep(i,1,q) {
        int l=qs[i].l,r=qs[i].r,t=qs[i].t;
        while (cur<=t) {
            _for(g,vec[cur]) seg.modify(1,g,g,a[g]*v[g]);
            cur++;
        }
        ans[qs[i].i]=seg.query(1,l,r).sum;
    }
