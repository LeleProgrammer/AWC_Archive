...
i32 main() {
    ...
    _sorta(s,1,m);
    _rep(i,1,m-1) add(abs(s[i]-s[i+1]));
    int ans=inf;
    _rep(i,1,m) {
        if (i!=1) del(abs(s[i]-s[i-1]));
        if (i!=m) del(abs(s[i]-s[i+1]));
        if (i!=1 && i!=m) add(abs(s[i-1]-s[i+1]));
        if (st.empty()) chmin(ans,1);
        else {
            int mx=*prev(st.end());
            del(mx);
            int a=mx/2;
            int b=mx-a;
            add(a),add(b);
            chmin(ans,*prev(st.end()));
            del(a),del(b);
            add(mx);
        }
        if (i!=1 && i!=m) del(abs(s[i-1]-s[i+1]));
        if (i!=1) add(abs(s[i]-s[i-1]));
        if (i!=m) add(abs(s[i]-s[i+1]));
    }
    write(ans);
    return 0;
}