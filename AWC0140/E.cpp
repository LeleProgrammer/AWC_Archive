...
inline void init() {
    _rep(i,0,9) f[0][i][Z+i]=1;
    _rep(i,1,N-1) _rep(j,0,9) _rep(k,0,9) _rep(t,0,M-1) {
        if (Z*2-(t-j)<0 || Z*2-(t-j)>=M) continue;
        f[i][j][t]+=f[i-1][k][Z*2-(t-j)];
    }
}
...
inline int solve(int n) {
    ...
    int ans=0;
    _rep(i,0,m-1) {
        _rep(j,1,9) _rep(k,Z-d,Z+d) ans+=f[i][j][k];
        // cout<<"cur: "<<ans<<endl;
    }
    _rep(i,1,vec.back()-1) _rep(k,Z-d,Z+d) ans+=f[m][i][k];
    int cur=Z+vec.back();
    int rev=0;
    _rrep(i,m-1,0) {
        _rep(j,0,vec[i]-1) _rep(k,Z-d,Z+d) {
            int dest=k-cur;
            if (!rev) dest=-dest;
            dest+=Z;
            // cout<<"test: "<<i<<" "<<j<<" "<<k<<" "<<dest<<endl;
            if (dest<0 || dest>=M) continue;
            ans+=f[i][j][dest];
        }
        if (rev) cur+=vec[i];
        else cur-=vec[i];
