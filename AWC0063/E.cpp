...
struct val {
    ...
    int lc,rc;
    int set;
    val(): l(0),r(0),cnt(0),lc(0),rc(0),set(0) {}
};
...
inline void pushup(val& u,val& l,val& r) {
    ...
    u.l=l.l;
    u.r=r.r;
    // cout<<"test: "<<u.l<<" "<<u.r<<": "<<u.cnt<<" "<<l.rc<<" "<<r.lc<<" "<<u.cnt<<endl;
}

inline void pushdown(val& u,val& l,val& r) {
    if (u.set) {
        l.set=r.set=u.set;
        l.lc=l.rc=r.lc=r.rc=u.set;
        l.cnt=r.cnt=1;
        u.set=0;
    }
}
...
i32 main() {
    read(n,q);
    _rep(i,1,n) read(c[i]);
    _rep(i,1,n) seg.modify(1,i,i,c[i]);
    while (q--) {
        int op,l,r;
        read(op,l,r);
        if (op==1) {
