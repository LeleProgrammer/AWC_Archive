...
i32 main() {
    read(n,m,k,t);
    _rep(i,1,n) _rep(j,1,m) read(s[i][j]);
    int l=1;
    _rep(r,1,m) {
        _rep(i,1,n) st[i].emplace(s[i][r]);
        while (r-l+1>k) {
            _rep(i,1,n) st[i].erase(LB2(st[i],s[i][l]));
            ++l;
        }
        _rep(i,1,n) chmax(mx[i],(*prev(st[i].end()))-(*st[i].begin()));
    }
    int ans=0;
    _rep(i,1,n) if (mx[i]>=t) ans++;
    write(ans);
    return 0;
}