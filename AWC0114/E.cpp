...
SEG<val,pushup,nullptr,int,modify,0> f[12],g[12];

i32 main() {
    _rep(i,0,11) f[i].reserve(N),g[i].reserve(N);
    read(n,m,k);
    _rep(i,1,n) read(v[i]),pre[i]=pre[i-1]+v[i];
    while (m--) {
        int l,r;
        read(l,r);
    ...
    f[0].modify(1,0,0,0);
    g[0].modify(1,0,0,0);
    _rep(r,1,n) _for(l,vec[r]) _rep(i,1,k) {
        int mxf=f[i-1].query(1,0,l-1).v;
        int mxg=g[i-1].query(1,l,r).v+pre[l-1];
        int nex=max(mxf,mxg)+pre[r]-pre[l-1];
        f[i].modify(1,r,r,nex);
        g[i].modify(1,r,r,nex-pre[r]);
    }
    write(f[k].query(1,0,n).v);
    return 0;
}