...
i32 main() {
    ...
    while (m--) {
        int p,d;
        read(p,d);
        vec.emplace_back(node{p,d,true});
    }
    while (k--) {
        int p;
        read(p);
        vec.emplace_back(node{p,-c,false});
    }
    _sort(vec);
    _for(t,vec) _rep(i,-1,1) if (t.pos+i>=0 && t.pos+i<=n) chk.emplace_back(t.pos+i);
    chk.emplace_back(0),chk.emplace_back(n);
    _sort(chk),_unique(chk);
    int l=0,r=-1;
    int ans=-inf;
    _for(ldest,chk) {
        int rdest=ldest+L;
        if (rdest>n) break;
        while (r+1<=(int)vec.size()-1 && vec[r+1].pos<=rdest) add(++r);
        while (l<=(int)vec.size()-1 && vec[l].pos<ldest) del(l++);
        chmax(ans,cur);
        if (!st.empty()) {
            int mn=*(st.begin());
            chmax(ans,cur-mn-R);
    ...
    cur=0,st.clear();
    r=(int)vec.size()-1,l=(int)vec.size();
    _riter(it,chk) {
        int rdest=*it;
        int ldest=rdest-L;
        if (ldest<0) break;
        while (l-1>=0 && vec[l-1].pos>=ldest) add(--l);
        while (r>=0 && vec[r].pos>rdest) del(r--);
        chmax(ans,cur);
        if (!st.empty()) {
            int mn=*(st.begin());
            chmax(ans,cur-mn-R);
