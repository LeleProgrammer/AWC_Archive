...
i32 main() {
    read(n,s,t);
    _rep(i,1,n) read(p[i],c[i],w[i]);
    _rep(i,0,N-1) _rep(j,0,N-1) _rep(k,0,M-1) f[i][j][k]=-inf;
    f[0][0][0]=0;
    _rep(i,1,n) _rep(j,0,i) _rep(k,0,s) {
        chmax(f[i][j][k],f[i-1][j][k]);
        if (k>=w[i] && j) chmax(f[i][j][k],f[i-1][j-1][k-w[i]]+p[i]-c[i]);
    }
    _rep(i,1,n) _rep(j,0,s) if (f[n][i][j]>=t) {
        write(i);
        return 0;
    }
    puts("-1");
