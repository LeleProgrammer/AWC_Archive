...
i32 main() {
    read(n,c);
    _rep(i,0,n-1) read(w[i]);
    _rep(i,0,M-1) f[i]=inf;
    f[0]=0;
    _rep(i,1,n) _rep(t,0,(1<<n)-1) if (popc(t)==i) {
        vector<int> vec;
        _rep(j,0,n-1) if (kthbit(t,j)) vec.emplace_back(j);
        _rep(p,0,(1<<vec.size())-1) {
            int sta=t;
            int sum=0;
            _rep(j,0,(int)vec.size()-1) if (kthbit(p,j)) {
                sta^=(1<<vec[j]);
                sum+=w[vec[j]];
                if (sum>c) break;
            }
            if (sum<=c) chmin(f[t],f[sta]+1);
        }
    }
    write(f[(1<<n)-1]);
    return 0;
