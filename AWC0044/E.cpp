...
i32 main() {
    ...
    f=new int*[n+5];
    _rep(i,0,n+4) {
        f[i]=new int[min(n,m)+5];
        _rep(j,0,min(n,m)+4) f[i][j]=0;
    }
    _rep(i,1,n) read(d[i]);
    if (d[1]<=1) f[1][1]=m;
    _rep(i,2,n) _rep(j,1,min(n,m)) {
        if (d[i]) {
            if (j>d[i]) f[i][j]=f[i-1][j];
            else if (j==d[i]) f[i][j]=(f[i-1][j]+f[i-1][j-1]*(m-(j-1)))%mod;
        } else {
            f[i][j]=(f[i-1][j]*j+f[i-1][j-1]*(m-(j-1)))%mod;
        }
    }
    int ans=0;
    _rep(i,1,min(n,m)) ans=(ans+f[n][i])%mod;
    write(ans);
    return 0;
}