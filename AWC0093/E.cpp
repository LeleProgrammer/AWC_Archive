...
inline void dfs(int u,int c,bool& ok) {
    col[u]=c;
    _graph(i,u) {
        if (!~col[e[i]]) dfs(e[i],c^1,ok);
        else if (col[e[i]]==col[u]) ok=false;
    }
}
...
i32 main() {
    ...
    while (q--) {
        ...
        if (*op=='A') {
            ...
            vec.emplace_back(vi3{u,v,val});
        } else {
            int k;
            read(k);
            vec[k].c=calc(vec[k-1].c);
        }
        Graph::restore_graph();
        _rep(i,1,n) p[i]=i,col[i]=-1;
        _for(t,vec) if (t.c==0) merge(t.a,t.b);
        _for(t,vec) if (t.c==1) add(find(t.a),find(t.b)),add(find(t.b),find(t.a));
        bool ok=true;
        _rep(i,1,n) if (!~col[find(i)]) dfs(find(i),0,ok);
        if (!ok) {
            puts("0");
            continue;
        }
        _for(t,vec) if (t.c==1) merge(t.a,t.b);
        int ans=0;
        _for(t,vec) if (t.c==2) {
            int a=t.a,b=t.b;
