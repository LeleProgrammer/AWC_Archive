...
i32 main() {
    ...
    readstr(s+1);
    readstr(t+1);
    _rrep(i,n,1) A=A*2+s[i]-'A';
    _rrep(i,n,1) B=B*2+t[i]-'A';
    _rep(i,0,M-1) dis[i]=inf;
    queue<int> q;
    q.emplace(A); dis[A]=0;
    vector<int> vec;
    _rep(i,1,n) if (n%i==0) vec.emplace_back(i);
    while (!q.empty()) {
        int u=q.front(); q.pop();
        if (flag[u]) continue;
        flag[u]=true;
