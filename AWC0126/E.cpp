...
inline void dfs(int u,int fa,int cur,int st) {
    sum[u]=arr[u];
    if (cur!=d) {
        _graph(i,u) {
            if (e[i]==fa) continue;
    ...
    if (st!=u) mp[pii{min(u,fa),max(u,fa)}].emplace_back(pii{st,sum[u]});
    else res[u]=sum[u];
}
...
i32 main() {
    ...
    // _rep(i,1,n) cout<<"res: "<<i<<" "<<res[i]<<endl;
    int ans=inf;
    _rep(i,1,n) _rep(j,i+1,n) {
        if (!mp.count(pii{i,j})) continue;
        int mx=0;
        auto& g=mp[pii{i,j}];
        _for(t,g) res[t.first]-=t.second;
        _rep(j,1,n) chmax(mx,res[j]);
        chmin(ans,mx);
        _for(t,g) res[t.first]+=t.second;
    }
    write(ans);
