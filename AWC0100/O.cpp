...
int n;
int cnt[N],s[N][N],f[N][N],f2[N][N];

i32 main() {
    read(n);
    _rep(i,1,n) read(cnt[i]),cnt[i]+=cnt[i-1],s[i][i]=i,s[i+n][i+n]=i+n;
    _rep(i,1,n) cnt[i+n]=cnt[i]+cnt[n];
    _rrep(i,n*2,1) {
        _rep(j,i+1,n*2) {
            int tmp=inf,tmp2=0;
            f2[i][j]=max(f2[i+1][j],f2[i][j-1])+cnt[j]-cnt[i-1];
            _rep(k,s[i][j-1],s[i+1][j]) if (tmp>f[i][k]+f[k+1][j]+cnt[j]-cnt[i-1]) {
                tmp=f[i][k]+f[k+1][j]+cnt[j]-cnt[i-1];
                tmp2=k;
            }
            f[i][j]=tmp;
            s[i][j]=tmp2;
