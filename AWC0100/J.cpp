...
int n,q,r;
int arr[N];
int dep[N],fa[N],sz[N],son[N],top[N],id[N],g[N],cnt;
...
inline void modify(val& u,const pii& k) {
    if (k.first==0) {
        u.v=k.second;
        return;
    }
...
SEG<val,pushup,pushdown,pii,modify> seg(N);

inline void modify_path(int u,int v) {
    while (top[u]!=top[v]) {
        if (dep[top[u]]<dep[top[v]]) swap(u,v);
        seg.modify(1,id[top[u]],id[u],pii{1,1});
        u=fa[top[u]];
    }
    if (u!=v) {
        if (dep[u]<dep[v]) swap(u,v);
        seg.modify(1,id[son[v]],id[u],pii{1,1});
    }
}
...
inline int query_path(int u,int v) {
    ...
    while (top[u]!=top[v]) {
        if (dep[top[u]]<dep[top[v]]) swap(u,v);
        // cout<<"chmin: "<<id[top[u]]<<" "<<id[u]<<" "<<seg.query(1,id[top[u]],id[u]).v<<endl;
        chmin(ans,seg.query(1,id[top[u]],id[u]).v);
        u=fa[top[u]];
    }
    if (u!=v) {
        if (dep[u]<dep[v]) swap(u,v);
        chmin(ans,seg.query(1,id[son[v]],id[u]).v);
    }
    return ans;
}
...
i32 main() {
    ...
    dfs1(1,0,1);
    dfs2(1,1);
    _rep(i,2,n) seg.modify(1,id[i],id[i],pii{0,arr[i]});
    // _rep(i,2,n) cout<<"test: "<<i<<" "<<id[i]<<" "<<g[i]<<endl;
    read(q);
    while (q--) {
        int u,v;
        read(u,v);
