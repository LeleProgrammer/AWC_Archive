...
i32 main() {
    read(n,m);
    _rep(i,0,m-1) read(c[i]);
    _rep(i,1,n) _rep(j,0,m-1) read(v[i][j]);
    _rep(j,0,m-1) {
        vector<int> pos;
        _rep(i,1,n) pos.emplace_back(i);
        sort(_all(pos),[&](const int& a,const int& b) {
            return v[a][j]>v[b][j];
        });
        _rep(i,1,min(n,m+5)) st.emplace(pos[i-1]);
    }
    int cur=0,las=1;
    _rep(i,0,1) _rep(j,0,P-1) f[i][j]=-inf;
    f[cur][0]=0;
    _for(i,st) {
        swap(cur,las);
        _rep(t,0,P-1) f[cur][t]=-inf;
        _rep(sta,0,(1<<m)-1) {
            chmax(f[cur][sta],f[las][sta]);
            _rep(j,0,m-1) if (!kthbit(sta,j)) if (v[i][j]-c[j]>=0) chmax(f[cur][setbit(sta,j)],f[las][sta]+v[i][j]-c[j]);
        }
    }
    int ans=0;
    _rep(i,0,P-1) chmax(ans,f[cur][i]);
    write(ans);
    return 0;
}