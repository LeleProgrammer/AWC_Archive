...
inline void init() {
    _rep(i,0,9) g[0][i][!i]=1;
    _rep(i,1,N-1) {
        _rep(j,0,9) {
            _rep(k,0,9) {
                if (j) {
                    f[i][j][0]=(f[i][j][0]+f[i-1][k][0]+abs(j-k)*g[i-1][k][0])%mod;
                    g[i][j][0]=(g[i][j][0]+g[i-1][k][0])%mod;
                } else {
                    f[i][j][1]=(f[i][j][1]+f[i-1][k][0]+abs(j-k)*g[i-1][k][0])%mod;
                    g[i][j][1]=(g[i][j][1]+g[i-1][k][0])%mod;
                }
                f[i][j][1]=(f[i][j][1]+f[i-1][k][1]+abs(j-k)*g[i-1][k][1])%mod;
                g[i][j][1]=(g[i][j][1]+g[i-1][k][1])%mod;
            }
        }
    }
}
...
inline int solve(int n) {
    ...
    int len=nums.size()-1;
    int ans=0;
    _rep(i,0,len-1) _rep(j,1,9) ans=(ans+f[i][j][0])%mod,ans=(ans+f[i][j][1]*2)%mod;
    _rep(i,1,nums[len]-1) ans=(ans+f[len][i][0])%mod,ans=(ans+f[len][i][1]*2)%mod;
    int cur=nums[len];
    bool zero=false;
    _rrep(i,len-1,0) {
        _rep(j,0,nums[i]-1) {
            if (zero) {
                ans=(ans+2LL*(f[i][j][0]+f[i][j][1])+2LL*(g[i][j][0]+g[i][j][1])*(calc(cur)+abs((cur%10)-j)))%mod;
            } else {
                ans=(ans+f[i][j][0]+g[i][j][0]*(calc(cur)+abs((cur%10)-j)))%mod;
                ans=(ans+2LL*f[i][j][1]+2LL*g[i][j][1]*(calc(cur)+abs((cur%10)-j)))%mod;
            }
        }
        cur=cur*10+nums[i];
        if (!nums[i]) zero=true;
